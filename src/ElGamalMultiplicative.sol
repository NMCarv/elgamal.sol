...
contract ElGamalMultiplicative {
    ...
    function homomorphicMultiplication(Ciphertext calldata ct1, Ciphertext calldata ct2, PublicKey calldata pk)
        external
        view
    ...
    /// @return newC1 The first component of the resulting ciphertext
    /// @return newC2 The second component of the resulting ciphertext
    function homomorphicDivision(Ciphertext calldata ct1, Ciphertext calldata ct2, PublicKey calldata pk)
        external
        view
        returns (BigNumber memory newC1, BigNumber memory newC2)
    {
        require(
            !BigNum.isZero(BigNumber(ct2.c1, false, BigNum.bitLength(ct2.c1))), "Cannot divide by zero ciphertext c1"
        );
        require(
            !BigNum.isZero(BigNumber(ct2.c2, false, BigNum.bitLength(ct2.c2))), "Cannot divide by zero ciphertext c2"
        );

        BigNumber memory bn_p = BigNumber(pk.p, false, BigNum.bitLength(pk.p));
        BigNumber memory bn_2 =
            BigNumber(abi.encodePacked(uint256(2)), false, BigNum.bitLength(abi.encodePacked(uint256(2))));

        // Compute inverse of ct2.c1: (c1')^{p-2} mod p
        BigNumber memory invC1 =
            BigNum.modexp(BigNumber(ct2.c1, false, BigNum.bitLength(ct2.c1)), BigNum.sub(bn_p, bn_2), bn_p);

        // Compute inverse of ct2.c2: (c2')^{p-2} mod p
        BigNumber memory invC2 =
            BigNum.modexp(BigNumber(ct2.c2, false, BigNum.bitLength(ct2.c2)), BigNum.sub(bn_p, bn_2), bn_p);
        ...
        returns (BigNumber memory m)
    {
        ...
        m = BigNum.modmul(BigNumber(ct.c2, false, BigNum.bitLength(ct.c2)), inv_h_r, bn_p);
    }
}
