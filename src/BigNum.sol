...
/**
 * @notice BigNum library for Solidity.
 */
library BigNum {
     ...
     */
    function verify(BigNumber memory bn) internal pure {
        uint256 msword;
    ...
    function toUint(BigNumber memory a) internal pure returns (uint256) {
     ...
     */
    function init(bytes memory val, bool neg, uint256 bitlen) internal view returns (BigNumber memory) {
     ...
     *  @return BigNumber instance
     */
    function init(bytes memory val, bool neg) internal view returns (BigNumber memory) {
     ...
     *  @return BigNumber instance
     */
    function init(uint256 val, bool neg) internal view returns (BigNumber memory) {
     ...
     */
    function add(BigNumber memory a, BigNumber memory b) internal pure returns (BigNumber memory r) {
     ...
     */
    function sub(BigNumber memory a, BigNumber memory b) internal pure returns (BigNumber memory r) {
        if (a.bitlen == 0 && b.bitlen == 0) return zero();
        bytes memory val;
        int256 compare;
        uint256 bitlen;
     ...
     */
    function mul(BigNumber memory a, BigNumber memory b) internal view returns (BigNumber memory r) {
     ...
     */
    function divVerify(BigNumber memory a, BigNumber memory b, BigNumber memory r) internal view returns (bool) {
        // first do zero check.
     ...
     */
    function pow(BigNumber memory a, uint256 e) internal view returns (BigNumber memory) {
     ...
     * @return r result BigNumber
     */
    function mod(BigNumber memory a, BigNumber memory n) internal view returns (BigNumber memory) {
     ...
     */
    function modinvVerify(BigNumber memory a, BigNumber memory n, BigNumber memory r) internal view returns (bool) {
     ...
     * @return int result
     */
    function cmp(BigNumber memory a, BigNumber memory b, bool signed) internal pure returns (int256) {
        int256 trigger = 1;
        if (signed) {
     ...
     * @return result BigNumber
     */
    function shr(BigNumber memory a, uint256 bits) internal view returns (BigNumber memory) {
        require(!a.neg);
        return _shr(a, bits);
    }
     ...
     * @return result BigNumber
     */
    function shl(BigNumber memory a, uint256 bits) internal view returns (BigNumber memory) {
        require(!a.neg);
        return _shl(a, bits);
    }
     ...
     */
    function bitLength(BigNumber memory a) internal pure returns (uint256) {
     ...
     */
    function bitLength(bytes memory a) internal pure returns (uint256 r) {
     ...
     * @return r uint bit length result.
     */
    function bitLength(uint256 a) internal pure returns (uint256 r) {
        assembly {
            switch eq(a, 0)
            case 1 { r := 0 }
            default {
     ...
     * @return uint - bit length of result.
     */
    function _sub(bytes memory max, bytes memory min) internal pure returns (bytes memory, uint256) {
        bytes memory result;
        uint256 carry = 0;
