...
contract ElGamalMultiplicativeTest is Test {
    ...
    function testEncryptDecryptSmallPrime() public {
        ...
        assertEq(c2.val, BigNum.modmul(bn_m, h_r, bn_p).val, "c2 should be m * h^r mod p");
    }
    ...
    function testDecryptMultiplicativeSmallPrime() public {
        ...
        BigNumber memory decryptedBN = elgamal.decrypt(Ciphertext(c1.val, c2.val), decryptionKey, pk);
        ...
        assertEq(actualBytes, expectedBytes, "Multiplicative decryption mismatch");
    }
    ...
    function testHomomorphicMultiplicationSmallPrime() public {
        ...
        (BigNumber memory newC1, BigNumber memory newC2) =
            elgamal.homomorphicMultiplication(Ciphertext(c1_1.val, c2_1.val), Ciphertext(c1_2.val, c2_2.val), pk);
    ...
    function testHomomorphicDivisionSmallPrime() public {
        ...
        (BigNumber memory c1_2, BigNumber memory c2_2) = elgamal.encrypt(abi.encodePacked(m2), r2, pk);
        (BigNumber memory newC1, BigNumber memory newC2) =
            elgamal.homomorphicDivision(Ciphertext(c1_1.val, c2_1.val), Ciphertext(c1_2.val, c2_2.val), pk);

        BigNumber memory bn_p = BigNumber(pk.p, false, BigNum.bitLength(pk.p));
        BigNumber memory bn_2 = BigNumber(abi.encodePacked(uint256(2)), false, 2);
        BigNumber memory invC1 = BigNum.modexp(c1_2, BigNum.sub(bn_p, bn_2), bn_p);
        BigNumber memory invC2 = BigNum.modexp(c2_2, BigNum.sub(bn_p, bn_2), bn_p);

        assertEq(newC1.val, BigNum.modmul(c1_1, invC1, bn_p).val, "newC1 mismatch");
        assertEq(newC2.val, BigNum.modmul(c2_1, invC2, bn_p).val, "newC2 mismatch");
    }
    ...
    function padTo32(bytes memory b) internal pure returns (bytes memory padded) {
